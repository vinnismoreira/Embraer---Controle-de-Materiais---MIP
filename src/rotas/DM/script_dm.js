...
// ======================
// 2️⃣ SALVAR IMAGEM NO LOCALSTORAGE E PREVIEW
// ======================
...
document.getElementById("image-upload").addEventListener("change", function (event) {
    ...
    reader.onload = function (e) {
        ...
        localStorage.setItem("anexo-imagem", imagemBase64);

        // Mostra a prévia no modal
        const preview = document.getElementById("preview");
        preview.innerHTML = `<img src="${imagemBase64}" alt="Prévia da imagem">`;
    };
    reader.readAsDataURL(file);
});

// ======================
// 3️⃣ AO CLICAR EM "SALVAR" -> ADICIONA NA TABELA
// ======================
document.getElementById("save-item-btn").addEventListener("click", function () {
    const rota = document.getElementById("route").value.trim();
    if (!rota) {
        alert("Selecione uma rota antes de salvar.");
        return;
    }
    ...
    // Coluna Nome (usa o valor da rota)
    const nomeCell = document.createElement("td");
    nomeCell.textContent = rota;

    // Coluna Respostas (por enquanto fica zerada)
    const respostasCell = document.createElement("td");
    respostasCell.textContent = "—";
    respostasCell.style.textAlign = "center";

    // Coluna Anexados
    const anexosCell = document.createElement("td");
    anexosCell.style.textAlign = "center";
    ...
    if (imagemBase64) {
        ...
        link.title = "Ver imagem anexada";
        anexosCell.appendChild(link);
    } else {
        anexosCell.textContent = "—";
    }

    // Monta linha
    row.appendChild(nomeCell);
    row.appendChild(respostasCell);
    row.appendChild(anexosCell);

    tbody.appendChild(row);

    // Salva lista de registros no localStorage
    const registros = JSON.parse(localStorage.getItem("registros")) || [];
    registros.push({ rota, imagem: imagemBase64 });
...
// ======================
// 0️⃣ ABRIR E FECHAR MODAL DE REGISTRO
// ======================
const modal = document.getElementById("item-modal");
...
// ======================
// 4️⃣ CARREGAR REGISTROS AO INICIAR A PÁGINA
// ======================
window.addEventListener("DOMContentLoaded", function () {
    const registros = JSON.parse(localStorage.getItem("registros")) || [];
    const tbody = document.getElementById("stock-table-body");

    registros.forEach(({ rota, imagem }) => {
        const row = document.createElement("tr");
